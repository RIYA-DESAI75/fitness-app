...
export default function Activeworkout() {
  ...
  // Reset the timer when screen is focused and no active workout (fresh start)
  useFocusEffect(
    React.useCallback(() => {
      // Only reset if there are no workout exercises (indicates a fresh start)
      if (workoutExercises.length === 0) {
        reset();
      }
    }, [workoutExercises, reset])
  );
  ...
  const saveWorkoutToDatabase = async () => {
    ...
    try { 
      // Use stopwatch to see total seconds for duration
      const durationInSeconds = totalSeconds;

      // Transform exercises data to match Sanity schema (no _type/_key for array items)
      const exercisesForSanity = await Promise.all(
        workoutExercises.map(async (exercise) => {
          ...
          // Only completed sets, convert to numbers, no _type/_key
          ...
          return {
            exercise: {
              ...
              _ref: exerciseDoc._id as string,
            },
            sets: setsForSanity,
          };
        })
      ...
      // Filter out any exercises with no completed sets
      ...
      // Save the workout document to Sanity via API
      ...
      <View className="bg-gray-800 px-6 py-4">
        <View className="flex-row items-center justify-between">
          ...
          <View className="flex-row items-center space-x-3 gap-2">
            {/* Weight Unit Toggle */}
            <View className="flex-row items-center bg-gray-700 rounded-lg p-1">
              <TouchableOpacity
                onPress={() => setWeightUnit("lbs")}
                className={`px-3 py-1 rounded-lg ${weightUnit === "lbs" ? "bg-blue-600" : ""}`}
              >
            ...
            >
              <Text className="text-white font-bold">End Workout</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
              ...
              <View key={exercise.id} className="mb-8">
                ...
                <View className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-3">
                          ...
                          <View className="flex-1 mx-2">
                            ...
                            <TextInput
                              ...
                              className={`border rounded-lg px-3 py-2 text-center ${
                                set.isCompleted
                                  ? "bg-green-100 border-gray-300 text-gray-500"
                  ...
                  <TouchableOpacity
                    ...
                    className="bg-blue-100 border-2 border-dashed items-center mt-2 border-blue-300 rounded-lg py-3"
                  >
